    ...
    async function loadAvailability() {
      try {
        ...
        if (res.ok) {
          ...
          setEnabledDays(slots.map((slot) => slot.dayOfWeek).sort())
          setStartTime(slots[0]?.startTime ?? '09:00')
          setEndTime(slots[0]?.endTime ?? '17:00')
        } else {
          setStatus(data.error ?? 'Entre novamente para carregar seus horarios.')
        }
      } finally {
    ...
    <main className="min-h-screen bg-[#f7f3ea] text-slate-950">
      ...
      <section className="mx-auto max-w-5xl px-5 py-8">
        <div className="rounded-lg border border-slate-200 bg-white p-6">
          ...
          {loading ? (
            <div className="rounded-lg bg-slate-100 px-4 py-3 text-sm font-bold text-slate-700">Carregando disponibilidade...</div>
          ) : (
