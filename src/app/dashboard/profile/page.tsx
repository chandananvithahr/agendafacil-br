...
const appUrl = (process.env.NEXT_PUBLIC_APP_URL ?? 'https://agendafacil.com.br').replace(/\/$/, '')
    ...
    async function loadProfile() {
      try {
        ...
        if (res.ok) {
          ...
          setStatus(data.error ?? 'Entre novamente para carregar seu perfil.')
        }
      } finally {
    ...
    <main className="min-h-screen bg-[#f7f3ea] text-slate-950">
      ...
      <div className="mx-auto grid max-w-5xl gap-6 px-5 py-8 lg:grid-cols-[1fr_320px]">
        <section className="rounded-lg border border-slate-200 bg-white p-6">
          ...
          {loading ? (
            <div className="rounded-lg bg-slate-100 px-4 py-3 text-sm font-bold text-slate-700">Carregando perfil...</div>
          ) : (
